...
def intent_agent(state):
    ...
    Query: {state['query']}
    """
    state["intent"] = llm.invoke(msg).content.lower()
    return state
...
def recommendation_agent(state):
    ...
    Recommend funds for profile:
    {state['profile']}
    """
    state["response"] = llm.invoke(msg).content
    return state
...
amount = st.sidebar.number_input("Amount", min_value=1000)
